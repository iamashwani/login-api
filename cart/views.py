...
from .serializers import ProfileSerializer, \
    ...
    walletserializer_add,GetResponceSerializer,Transcationserializer,TranscationHistoryserializer
...
def transcationmoney(request, pk):
    if request.method == 'POST':
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            qs.description = des
            if qs.total_amount > amount:
                qs.total_amount = qs.total_amount + qs.amount
                qs.save()
            elif qs.total_amount < amount:
                qs.total_amount = qs.total_amount - amount
            qs = Transcations.objects.create(wallet=qs, amount=qs.amount,description=qs.description,total_amount=qs.total_amount )
            qs.save()
        json_data = serializer.data
        x = GetResponceSerializer(json_data)
        x = {**x.data, **json_data}
